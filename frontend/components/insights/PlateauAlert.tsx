...
export const PlateauAlert: React.FC<PlateauAlertProps> = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={!clickable}
      className={`w-full flex items-center gap-3 p-2 rounded-lg text-left bg-amber-500/3 border border-amber-500/3 ${clickable ? 'cursor-pointer border border-transparent hover:border-slate-600/40 transition-colors' : 'cursor-default'}`}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {suggestion && (
          <div className="text-[11px] sm:text-xs text-amber-400 mt-1 line-clamp-1 flex items-center gap-1">
      ...
      <div className="text-right">
        {isBodyweightLike ? (
          <>
            ...
            <div className="text-[11px] sm:text-xs text-slate-500">Bodyweight</div>
          </>
        ) : (
          <>
            ...
            <div className="text-[11px] sm:text-xs text-slate-500">×{lastReps} reps</div>
          </>
        )}
      </div>
    </button>
